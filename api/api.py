...
app.config['MONGO_DBNAME'] = f"mongodb+srv://username:Bisleri@cluster0.qv3wn6u.mongodb.net/Data?retryWrites=true&w=majority&appName=Cluster0"
...
def register():
    ...
    final = {'ID': count+1, 'Name': data['name'].lower(), 'Email': data['email'].lower(), 'Hash': salt, 'Type': data['category'].lower}
    if not utils.signed_up(name=data['name'].lower(), email=data['email'].lower(), category=data['category'].lower()):
        ...
        return jsonify({'message': 'Registered Successfully!', 'category': '', 'name': '', ''}), 201
    else:
...
def investorInfo():
    try:
        ...
        final = {'ID': db.auth.count(), 'preferences': data['preferences'], 'subPreferences': data['subPreferences'], 'investmentType': data['investmentType'], 'country': data['country'], 'description': data['description'], 'pastInvestments': data['pastInvestments'], 'validation': data['validation']}
        collection.insert_one(final)
...
def projectInfo():
    try:
        ...
        final = {'ID': db.auth.count(), 'project': data['project'], 'intro': data['intro'], 'teamSize': data['teamSize'], 'category': data['category'], 'subCategory': data['subCategory'], 'description': data['description'], 'bestAchievements': data['bestAchievements'], 'links': data['links'], 'funding': data['expectedFunding'], 'pitch': data['pitch'], 'expectedROI': data['expectedROI'], 'country': data['country']}
        collection.insert_one(final)
